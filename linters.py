...
class I18nLinter(Linter):
    ...
    def run_all(self, filenames, contents_callback):
        strings = {}
        new_contents = {}
        not_sorted = set()
        lang_path = set()
        ...
        for language in langs:
            lang_path.add('%s/%s.lang' % (_TEMPLATES_PATH, language))

        for filename in lang_path:
        ...
        for key, values in strings.items():
            ...
            if missing_languages:
                print('%s%s%s' % (_HEADER, key, _NORMAL),
                    file=sys.stderr)

                for lang in sorted(languages):
                    if lang in values:
                        print('\t%s%-10s%s %s' %
                            (_OKGREEN, lang, _NORMAL, values[lang]),
                            file=sys.stderr)
                    else:
                        print('\t%s%-10s%s missing%s' %
                            (_OKGREEN, lang, _FAIL, _NORMAL), file=sys.stderr)
            ...
            if key == 'locale':
                ...
                values['pseudo'] = _pseudoloc(values['en'])

        for language in langs:
            template_path = '%s/%s.lang' % (_TEMPLATES_PATH, language)
            js_lang_path = '%s/lang.%s.js' % (_JS_TEMPLATES_PATH, language)
            json_lang_path = '%s/lang.%s.json' % (_JS_TEMPLATES_PATH, language)

            js_contents = contents_callback(js_lang_path)
            js_new_contents = _generate_javascript(language, strings)
            if js_contents.decode('utf-8') != js_new_contents:
                print('Entries in %s do not match the .lang file.' %
                    js_lang_path, file=sys.stderr)
                new_contents[js_lang_path] = js_new_contents

            json_contents = contents_callback(json_lang_path)
            json_new_contents = json.dumps(_generate_json(language, strings),
                sort_keys=True, indent='\t')
            if json_contents.decode('utf-8') != json_new_contents:
                print('Entries in %s do not match the .lang file.' %
                    json_lang_path, file=sys.stderr)
                new_contents[json_lang_path] = json_new_contents

            if language == 'pseudo':
                new_contents[template_path] = _generate_pseudo(language, strings)

        return new_contents, ['i18n']
